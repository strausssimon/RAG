...
Testet ein gespeichertes Keras-Modell mit unabhängigen Testdaten.
...
class ModelTester:
    def __init__(self, model_path="models/mushroom_7class_resnet_cnn_80_20_split.keras"):
        self.model_path = Path(model_path)
        self.test_data_path = Path("data/test_mushrooms_randomized/inaturalist")
        ...
        for img_file, label in tqdm(zip(test_files, test_labels), desc="Loading test images", total=len(test_files)):
            try:
                ...
                if img is not None:
                    ...
                    print(f"   ⚠️ Warnung: Kann {img_file.name} nicht laden")
...
def main():
    ...
    tester = ModelTester(
        model_path="models/mushroom_7class_resnet_cnn_80_20_split.keras"
    )
